...
This document records the intermediate research process for strategies that use **daily changes in existing chip detail data**. It is intentionally separate from implementation code. No rule in this document should be treated as a production trading strategy until it has been backtested against this project's target multi-horizon observation windows.
...
Status: mostly project hypothesis. Must be proven with multi-horizon validation before use.
...
- Do short-, medium-, and long-horizon observations need different rule weights?
